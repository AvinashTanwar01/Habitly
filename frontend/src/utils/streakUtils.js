...
export function getEffectiveToday(now = new Date(), userResetTime = '00:00') {
...
export function isScheduled(dateStr, scheduleType, scheduleDays = []) {
  const d = parseDate(dateStr)
  const dow = d.getDay()
...
export function isScheduledDay(dateStr, scheduleType, scheduleDays = []) {
  return isScheduled(dateStr, scheduleType, scheduleDays)
}
...
export function completionRate(completions, scheduleType, scheduleDays, days = 30) {
  const today = formatDate(new Date())
  const start = addDays(today, -(days - 1))
  const scheduled = getScheduledDates(start, today, scheduleType, scheduleDays)
  if (!scheduled.length) return 0
  const doneSet = new Set(completions.filter((c) => c.isDone).map((c) => c.date))
  return Math.round((scheduled.filter((d) => doneSet.has(d)).length / scheduled.length) * 100)
}
